...
<%-- 	   </c:if> --%>
	    ...
	    <!-- 이전 페이지로 가기 -->
	   <c:if test="${paging.curPage ne 1 }">
	   <li><a href="${pageContext.request.contextPath }/teacher/blackList?curPage=${paging.curPage - 1 }&search=${param.search }">&lt;</a>
	   </c:if>
	   ...
	   <li class="active"><a href="${pageContext.request.contextPath }/teacher/blackList?curPage=${i }&search=${param.search }">${i }</a></li>
	   </c:if>
	   
	   <!-- 현재 페이지가 아니라면 평소 모습-->
	   <c:if test="${paging.curPage ne i }">
	   <li><a href="${pageContext.request.contextPath }/teacher/blackList?curPage=${i }&search=${param.search }">${i }</a></li>
	   </c:if>
