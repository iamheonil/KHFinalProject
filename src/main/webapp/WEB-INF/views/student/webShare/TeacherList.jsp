...
function webshareClick(CONN_LESSON_NO){
	...
	$(location).attr("href", "${pageContext.request.contextPath}/student/webshare?no="+no);
}
...
function lessonEnd(connLessonNo){
	...
	var chk = confirm("선생님과의 과외를 끝내시겠습니까?");
	                            ...
	                            <tbody>
	                               	<c:forEach items="${connectedLessonList }" var="lesson" >
										<tr>
											...
											<td><a href="#"  onclick="lessonModal(${lesson.LESSON_NO});" title="과외 상세 보기">${lesson.LESSON_TITLE}</a></td>
											<td><a href="#" onclick="StudentModal(${lesson.TEACHER });" title="학생 프로필 보기">${lesson.USER_ID}</a></td>
											...
											<td><a id="xicon" href="javascript:void(0);" onclick="lessonEnd(${lesson.CONN_LESSON_NO});" class="glyphicon glyphicon-remove"></a></td>
										</tr>
									</c:forEach>
	                            </tbody>
