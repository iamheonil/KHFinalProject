...
<%--    <c:if test="${paging.curPage gt paging.pageCount  }"> --%>
   <li><a href="url작성?curPage=1&&search=${param.search }">&laquo;</a></li>
   </c:if>
...
<%--    </c:if> --%>
   ...
   <!-- 이전 페이지로 가기 -->
   <c:if test="${paging.curPage ne 1 }">
   <li><a href="url작성?curPage=${paging.curPage - 1 }&search=${param.search }">&lt;</a>
   </c:if>
   ...
   <li><a href="url작성?curPage=${i }&search=${param.search }">${i }</a></li>
   </c:if>
...
<%--    </c:if> --%>
    ...
    <!-- 마지막 페이지로 가기 -->
   <c:if test="${paging.curPage ne paging.totalPage }">
   <li><a href="url작성?curPage=${paging.totalPage }&search=${param.search }">&raquo;</a></li>
   </c:if>
